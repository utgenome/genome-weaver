 ...
 * </pre>
 *
 * If you need to retrieve the entire sequence as a single String, use [[utgenome.weaver.core.FASTAEntryReader#sequence]].
 *
 * @author leo
 */
object FASTA extends Logger {
   ...
   * @param in
   */
  private[weaver] class SequenceStream(in: FASTAReader) {
  ...
  private[weaver] class TarFileLineReader(in: InputStream, bufferSize: Int = DEFAULT_BUFFER_SIZE) extends FASTAReader {
  ...
  private[weaver] def read[A](lineReader: FASTAReader)(f: Stream[FASTAEntryReader] => A): A = {
...
/**
 * Reader of each FASTA entry. Note that [[utgenome.weaver.core.FASTAEntryReader#lines]] is evaluated lazily.
 * @param description
 * @param ss
 */
class FASTAEntryReader(val description: String, private val ss: FASTA.SequenceStream) {
  ...
  /**
   * Extract the entire sequence. This result can be large. If you want to process genome sequences line by line, use [[utgenome.weaver.core.FASTAEntryReader#lines]] method.
   */
